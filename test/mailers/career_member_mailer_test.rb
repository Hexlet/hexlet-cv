...
class CareerMemberMailerTest < ActionMailer::TestCase
  ...
  test 'career member finish' do
    ...
    user = career_member.user
    mail = CareerMemberMailer.with(career_member:, user:).career_member_finish_email
    assert { user.email == mail.to.first }
  end

  test '#next_career_step_open_source' do
    mail = CareerMemberMailer.with(career_member: @career_member, user: @user).next_step_open_source_email
    assert { @user.email == mail.to.first }
  end
end
